...
//       .then((response) => {
//         setMeetingRooms(response.data.payload);
//         localStorage.setItem('selectedStartTime', startDate);
//         localStorage.setItem('selectedEndTime', endDate);
//       })
//       .catch((error) => console.error('catch', error));
//   };
...
//         {meetingRooms.length ? meetingRooms.map((room, index) => (
...
function Home() {
  ...
  // Function to paginate the meeting rooms
  const paginate = (meetingRooms, currentPage, roomsPerPage) => {
    const startIndex = (currentPage - 1) * roomsPerPage
    const endIndex = startIndex + roomsPerPage
    return meetingRooms.slice(startIndex, endIndex)
  }

  const renderedMeetingRooms = paginate(meetingRooms, currentPage, roomsPerPage)

  const handleNextPage = () => {
    if (currentPage < Math.ceil(meetingRooms.length / roomsPerPage)) {
  ...
  return (
    <div>
      ...
      <div className='roomList'>
        {renderedMeetingRooms.length
          ? renderedMeetingRooms.map((room, index) => (
              <Link to={`/meetingrooms/${room.id}`} key={index}>
                <div className='roomList__container'>
      ...
      <div className='pagination'>
        ...
        <button
          ...
          disabled={
            currentPage === Math.ceil(meetingRooms.length / roomsPerPage)
          }
        >
          Next
        </button>
