...
meetingRooms.get('/', async (req, res) => {
  try {
    ...
    try {
      ...
      res
        .status(404)
        .json({ sucesss: false, message: 'no meeting room found!' })
    }
  } catch (error) {
...
meetingRooms.get('/:meetingRoom_id/bookings', async (req, res) => {
  ...
  try {
    ...
    if (getFutureBookings) {
      ...
      res
        .status(404)
        .json({ error: 'No future bookings for the specified meeting room' })
    }
  } catch (error) {
    res
      .status(500)
      .json({ error: 'An error occurred while fetching future bookings' })
  }
})
